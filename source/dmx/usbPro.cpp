...
uint8_t FTDI_RxDMX(uint8_t label, unsigned char *data, uint32_t expected_length)
{
	...
	if (bytes_read != length)
		return NO_RESPONSE;
	// Check The End Code
	if (buffer[length - 1] != DMX_END_CODE)
		return NO_RESPONSE;
	// Copy The Data read to the buffer passed
	memcpy(data, buffer, expected_length);
	return TRUE;
...
uint16_t FTDI_OpenDevice(int device_num)
{
	...
	do
	{
		printf("\n------ D2XX ------- Opening [Device %d] ------ Try %d\n", device_num, tries);
		ftStatus = FT_Open(device_num, &device_handle);
		std::cout << "STATUS : " << FT_Open << std::endl;
		// ftStatus = FT_OK;
		usleep(750000);
		tries++;
...
void sendDMX(unsigned char *dmxData, int length)
{
	FTDI_SendData(SET_DMX_TX_MODE, dmxData, length);
}

void startDMX(unsigned char *myDmx)
{
	FT_STATUS ftStatus;
	WORD wVID = 0x0403;
	WORD wPID = 0x6001;
